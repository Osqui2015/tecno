...
<template>
    ...
    <div v-else-if="stats" class="space-y-6">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="card p-5">
                <p class="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 mb-2">Revenue total</p>
                <p class="text-2xl font-black text-slate-800">{{ formatPrice(stats.revenue.total) }}</p>
                <p class="text-[10px] text-emerald-600 font-bold mt-1">
                    +{{ formatPrice(stats.revenue.this_month) }} este mes
                </p>
            </div>
            <div class="card p-5">
                <p class="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 mb-2">Pedidos totales</p>
                <p class="text-2xl font-black text-slate-800">{{ stats.orders.total }}</p>
                <p class="text-[10px] text-slate-400 font-bold mt-1">
                    {{ stats.orders.pending }} pendiente(s)
                </p>
            </div>
            <div class="card p-5">
                <p class="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 mb-2">Ticket promedio</p>
                <p class="text-2xl font-black text-slate-800">{{ formatPrice(stats.revenue.avg_ticket) }}</p>
                <p class="text-[10px] text-slate-400 font-bold mt-1">por pedido no cancelado</p>
            </div>
            <div class="card p-5">
                <p class="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 mb-2">Productos</p>
                <p class="text-2xl font-black text-slate-800">{{ stats.products.total }}</p>
        ...
        <div class="grid lg:grid-cols-3 gap-6">
            <!-- Embudo por estado -->
            <div class="card p-6">
                <h3 class="font-bold text-sm mb-4">Pedidos por estado</h3>
                ...
                <div v-else class="space-y-3">
                    <div v-for="(count, status) in stats.orders.by_status" :key="status">
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-xs font-bold text-slate-700">{{ statusLabel[status] || status }}</span>
            ...
            <div class="card p-6 lg:col-span-2">
                <h3 class="font-bold text-sm mb-4">Top 5 productos más vendidos</h3>
                ...
                <ul v-else class="space-y-2">
                    <li
                        ...
                        :key="p.id"
                        class="flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50"
                    >
                        <span class="w-6 h-6 rounded-full bg-brand-100 text-brand-700 flex items-center justify-center font-black text-xs">
                            {{ idx + 1 }}
                        </span>
                        ...
                        <span class="text-sm font-black text-slate-800">{{ formatPrice(p.revenue) }}</span>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="grid sm:grid-cols-2 gap-4">
            <router-link :to="{ name: 'admin-orders', query: { status: 'pending' } }" class="card p-5 hover:shadow-md transition-shadow flex items-center justify-between">
                <div>
                    <p class="font-bold text-sm">Revisar pedidos pendientes</p>
                    <p class="text-xs text-slate-500">{{ stats.orders.pending }} esperando confirmación</p>
                </div>
            ...
            <router-link :to="{ name: 'admin-products' }" class="card p-5 hover:shadow-md transition-shadow flex items-center justify-between">
                <div>
                    <p class="font-bold text-sm">Gestionar catálogo</p>
                    <p class="text-xs text-slate-500">{{ stats.products.total }} productos · {{ stats.products.out_of_stock }} sin stock</p>
                </div>
                <SvgIcon name="chevron-right" size="1rem" class="text-slate-400" />
            </router-link>
        </div>
