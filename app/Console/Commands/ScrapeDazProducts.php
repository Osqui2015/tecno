 ...
 *
 * Uso:
 *   php artisan daz:scrape                      Scrapea todas las páginas (delay 1s)
 *   php artisan daz:scrape --pages=2            Solo las primeras 2 páginas
 *   php artisan daz:scrape --delay=2            2 segundos entre requests
 *   php artisan daz:scrape --dry-run           No guarda, solo muestra qué traería
 *   php artisan daz:scrape --fresh              Vacía productos externos antes
 *   php artisan daz:scrape --category=hogar     Solo productos de esa categoría
 */
class ScrapeDazProducts extends Command
{
    protected $signature = 'daz:scrape
    ...
    private DazScraperService $scraper;
    ...
    public function handle(DazScraperService $scraper): int
    {
        ...
        $this->newLine();
        $this->info('╔══════════════════════════════════════════╗');
        $this->info('║   DAZ IMPORTADORA — SCRAPER DE PRODUCTOS ║');
        $this->info('╚══════════════════════════════════════════╝');
        $this->newLine();
        ...
        if ($fresh && ! $dryRun) {
            if (! $this->confirm('¿Borrar TODOS los productos con origin=daz antes de empezar?', false)) {
                $this->info('Operación cancelada por el usuario.');
                return self::SUCCESS;
            }
            $this->warn('🗑️  Vaciando productos Daz previos...');
            $deleted = Product::where('origin', 'daz')->delete();
            $this->info("   Eliminados: {$deleted} productos");
        }

        // Snapshot de external_ids existentes (solo Daz) para detectar "missing" después
        $existingExternalIds = Product::where('origin', 'daz')
            ->pluck('external_id')
            ->toArray();
        $this->info("📦 Productos Daz existentes: " . count($existingExternalIds));
        ...
        try {
            ...
            Log::error('DazScraper fatal', ['error' => $e->getMessage()]);
            return self::FAILURE;
        }
        ...
        // Marcar last_seen_at y activar todos los scrapeados (solo Daz)
        if (! empty($foundIds)) {
    ...
    private function saveProducts(array $products): array
    {
        ...
        // Pre-cache de IDs existentes (solo Daz)
            ...
            foreach ($products as $p) {
                        ...
                        if (! isset($categoryCache[$firstCat])) {
                            ...
                            if (! $cat) {
                                $cat = Category::create([
                                    'name'       => $firstCat,
                                    'slug'       => $catSlug,
                                    'description' => 'Importado de Dazimportadora',
                                ]);
                            }
                ...
                if (! $categoryId) {
                    $categoryId = Category::firstOrCreate(
                        ['slug' => 'importados'],
                        ['name' => 'Importados', 'description' => 'Productos importados de Dazimportadora']
                    )->id;
                }
                ...
                if (isset($existingByExt[$p['external_id']])) {
                    // Actualizar usando save() para que dispare los eventos del modelo
